...
const createGame = async game => {
    if (!verifyGameData(game)) return {success: false, error: "invalid game data"}
...
const getAllGames = async userID => {
    if (!userID) return {success: false, error: "invalid userID"}
    ...
    try {
        ...
        else return {success: false, error: res.status}
    } catch (error) {
        console.error(error);
...
const verifyGameData = (game) => {
    ...
    return (
          ...
          typeof snippet_id  !== undefined&&
          typeof total_time !== undefined &&
          typeof total_characters !== undefined &&
          typeof wpm_avg !== undefined &&
          typeof accuracy !== undefined &&
          typeof num_mistakes !== undefined &&
          wpm_data 
    )
}
