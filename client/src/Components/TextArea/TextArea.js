...
const Word = props => {
  const { word, index, cursor, userInput, lineIndex, wordActive } = props

  let className = ''
  const lettersMapper =
    wordActive && userInput.length > word.length
      ? userInput.split('')
      : word.split('')
  ...
  const correct =
    wordActive && userInput.length > word.length
      ...
      : (l, i) =>
          l === userInput[i] || !wordActive || i > cursor.letterIndex.current //inactive or default
  ...
  className =
    index < cursor.wordIndex.current && lineIndex <= cursor.lineIndex.current
      ? (className = 'visited')
      : (className = '')
...
export default function TextArea (props) {
  ...
  const handleSpecialKey = event => {
    // Space key handler
    if (
      atEndOfWord(currWord, userInput) &&
